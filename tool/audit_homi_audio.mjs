...
const domain = json('deliverables/homi-runtime-domain-audit.json');
...
for (const [text,source,anchor,family] of [
  ...
  ['Cô chưa nghe thấy con nói. Con nói lại nhé.', 'lib/features/conversation/presentation/conversation_controller.dart','_unclearSpeechMessage', 'conversation_unclear'],
...
const audioMatches = (text,locale) => prompts.filter(p => p.enabled && p.manifestEnabled &&
  p.text===text.trim() && (p.locale===locale || (p.lookupLocales??[]).includes(locale)));
const rows = [...entries.values()].map(row => {
  ...
  return {...row, scopes:[...new Set(row.occurrences.map(o=>o.scope))],
    audioStatus:!matches.length ? 'missing_exact_audio' : matches.length>1 ? 'ambiguous_audio_match' :
      Object.entries(check).some(([k,v])=>k!=='asset' && !v) ? 'invalid_audio' : 'ready',
...
const directLinkChecks = directLinks.map(link => {
  ...
  const match = prompts.find(p=>p.asset===asset);
  return {...link, asset, exists:asset ? existsSync(resolve(root,asset)) : null,
    bundled:asset ? isBundled(asset) : null, transcriptMatchesManifest:match ? match.text===link.text.trim() && match.locale===link.locale : false};
});
...
const summary = {
  date:'2026-09-16', scope:'local working tree; code/catalog audit, not device/acoustic verification',
  ...
  audioFiles:assetChecks.size, audioIntegrityFailures:[...assetChecks.values()].filter(c=>!c.exists||!c.bundled||!c.checksumValid||!c.withinRuntimeLimits),
  directLinks:directLinks.length, directLinkFailures:directLinkChecks.filter(c=>!c.exists||!c.bundled||!c.transcriptMatchesManifest),
...
const save = (name,value) => writeFileSync(resolve(root,`deliverables/${name}`),`${JSON.stringify(value,null,2)}\n`);
...
save('homi-speech-provenance.json', [...sourceFiles, ...manifests.map(m=>m.path),
  'assets/data/listening_lessons.json','pubspec.yaml',
...
writeFileSync(resolve(root,'deliverables/homi-speech-inventory.csv'),'\uFEFF'+[
