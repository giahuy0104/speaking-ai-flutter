...
# Share the immutable text cache with sentence audio. Identical model, voice,
# speed, locale and text must produce one clip rather than another API call.
...
$packByAge = @{
  ...
  '6-7' = 'challenge-6-7'
  '8-10' = 'challenge-8-10'
  '11-12' = 'challenge-11-12'
  '13-15' = 'challenge-13-15'
}
$manifestNameByAge = @{
  '3-5' = 'challenge_3_5_audio.json'
  '6-7' = 'challenge_6_7_audio.json'
  '8-10' = 'challenge_8_10_audio.json'
  '11-12' = 'challenge_11_12_audio.json'
  '13-15' = 'challenge_13_15_audio.json'
}
  ...
  foreach ($topic in $group.topics) {
    foreach ($lesson in $topic.lessons) {
      foreach ($challenge in $lesson.challengeBank) {
        foreach ($language in @('vi', 'en')) {
          $text = if ($language -eq 'vi') { $challenge.prompt } else { $challenge.correctAnswer }
          $normalized = Get-NormalizedText ([string]$text)
          if ([string]::IsNullOrWhiteSpace($normalized)) {
            throw "Challenge $($challenge.id) has empty $language text."
          }
...
Write-Output "Unique challenge clips: $($requests.Count); missing from cache: $($missingRequests.Count)."
...
foreach ($group in $catalog.groups) {
  ...
  if ([string]::IsNullOrWhiteSpace($pack)) {
    throw "No challenge pack is configured for age group $ageKey."
  }
  ...
  foreach ($topic in $group.topics) {
    foreach ($lesson in $topic.lessons) {
      foreach ($challenge in $lesson.challengeBank) {
        $challengeId = ([string]$challenge.id).Trim()
        foreach ($language in @('vi', 'en')) {
          $isPrompt = $language -eq 'vi'
          $text = if ($isPrompt) { $challenge.prompt } else { $challenge.correctAnswer }
          $normalized = Get-NormalizedText ([string]$text)
          $suffix = if ($isPrompt) { 'prompt.vi' } else { 'answer.en' }
          $key = "listening.challenge.$challengeId.$suffix"
          $fileName = "$key.mp3"
...
Write-Output 'Listening challenge audio generation completed.'
