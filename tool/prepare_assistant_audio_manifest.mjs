...
const previousById = new Map(manifest.prompts.map(p => [p.id, p]));
...
function add(id, text, source, language = 'vi') {
  ...
  if (!entry) {
    const previousEntry = previous.get(text);
    let assignedId = id;
    const occupied = previousById.get(assignedId) ?? [...prompts.values()].find(p=>p.id===assignedId);
    if (!previousEntry && occupied && occupied.text !== text) {
      assignedId = `${id}-${createHash('sha256').update(text).digest('hex').slice(0,8).toUpperCase()}`;
    }
    entry = { ...(previousEntry ?? { id: assignedId, text, locale: language === 'en' ? 'en-US' : 'vi-VN',
      version: 1, enabled: false, asset: `assets/audio/MAIN/${assignedId}.${language}.v1.mp3` }),
...
add('CONVERSATION-UNCLEAR', 'Cô chưa nghe thấy bạn nói. Bạn nói lại nhé.', 'lib/features/conversation/presentation/conversation_controller.dart');
...
for (const entry of previous.values()) {
  if (entry.batch?.startsWith('homi-runtime-gap')) prompts.set(entry.text, entry);
}
manifest.prompts = [...prompts.values()];
const duplicateValues = key => [...new Set(manifest.prompts.map(p => p[key])
  .filter((value,index,all) => all.indexOf(value)!==index))];
const duplicateIds = duplicateValues('id');
const duplicateAssets = duplicateValues('asset');
if (duplicateIds.length || duplicateAssets.length) {
  throw new Error(`Generated IDs/assets collide with an existing entry: ${JSON.stringify({duplicateIds,duplicateAssets})}`);
}
