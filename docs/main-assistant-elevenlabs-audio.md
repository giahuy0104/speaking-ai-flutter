...
Update 2026-09-17: runtime reconciliation adds 6 Vietnamese recordings plus one exact lookup alias (MAIN now 225; combined total 2,866). They are isolated into four small groups, each with its own build switch and TTS fallback. See [gap-6 delivery](homi-gap6-elevenlabs-2026-09-17.md).
...
# Generate and activate one reviewed prompt only.
node tool/generate_main_assistant_audio.mjs --id PROMPT-ID --generate --neutral-source-speed --api-key-file "C:/path-outside-repo/elevenlabs-key.txt"
node tool/generate_all_assistant_audio.mjs --generate --id PROMPT-ID
```
...
- MAIN navigation only: `--dart-define=HOMI_ASSISTANT_AUDIO_MAIN_NAVIGATION=false`.
- Translation controls only: `--dart-define=HOMI_ASSISTANT_AUDIO_TRANSLATION_CONTROLS=false`.
- Challenge cue only: `--dart-define=HOMI_ASSISTANT_AUDIO_CHALLENGE_CUES=false`.
- Conversation recovery only: `--dart-define=HOMI_ASSISTANT_AUDIO_CONVERSATION_RECOVERY=false`.
